...
public class UIAtlasMaker : EditorWindow
{
	...
	List<Texture> GetSelectedTextures ()
	{
		...
		if (Selection.objects != null && Selection.objects.Length > 0)
		{
			...
			//SD: not parsing scene objects! too slow!
			foreach (Object o in Selection.objects)
			{
				string objectName = o.ToString();
				if (objectName.Contains(" (UnityEngine.SceneAsset)"))
				{
					Debug.LogError("Selected scene! Not parsing scene objects! too slow!");
	...
	//sd: Dictionary, because string can contain int!!
