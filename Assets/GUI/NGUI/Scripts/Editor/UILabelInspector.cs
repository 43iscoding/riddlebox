...
public class UILabelInspector : UIWidgetInspector
{
	...
	protected override bool ShouldDrawProperties ()
	{
		...
		EditorGUI.BeginDisabledGroup(!isValid);
		{
			...
			//sd: dont show gradients
			if (mLabel.applyGradient)
			{
				//sd
				NGUIEditorTools.DrawProperty("Diagonal", serializedObject, "mDiagonalGradient",GUILayout.Width(95f));
			...
			if (mLabel.effectStyle != UILabel.Effect.None)
			{
				EditorGUI.BeginDisabledGroup(!sp.hasMultipleDifferentValues && !sp.boolValue);
				{
					...
					//sd: additional styles
					if (mLabel.effectStyle == UILabel.Effect.OutlineGradient || mLabel.effectStyle == UILabel.Effect.ShadowGradient || mLabel.effectStyle == UILabel.Effect.OutlineDiagonalGradient || mLabel.effectStyle == UILabel.Effect.ShadowDiagonalGradient)
					{
						NGUIEditorTools.DrawProperty("", serializedObject, "mEffectColorBottom", GUILayout.MinWidth(10f));
					}
					GUILayout.EndHorizontal();
