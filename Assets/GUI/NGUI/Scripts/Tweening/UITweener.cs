...
public abstract class UITweener : MonoBehaviour
{
	...
	public void Sample (float factor, bool isFinished)
	{
		...
		else if (method == Method.BubblingIn)
		{
			//new scaling modes
			val = BubblingLogic(val);
		}
		else if (method == Method.BubblingOut)
		{
			val = 1f - BubblingLogic(1f - val);
		}
	...
	//sd new tween effects
	float BubblingLogic(float val)
	{
		float val2 = Mathf.Sin(Mathf.PI * val);;
		float val3 = Mathf.Sin(Mathf.PI * val/2f);;
		float result = (val + val2 * val3);
		//Debug.Log(val + " => " + val2 + " -> " + result);

		float t = val;
		result = 2f * t * (1 - t) * 1.5f + t * t;
