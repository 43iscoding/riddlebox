...
public class UILabel : UIWidget
{
	public enum Effect
	{
		...
		Shadow,
		Outline,
		//SD outline with Shadow
		OutlineShadow,
		//sd outline with gradient
		ShadowGradient,
		OutlineGradient,
		ShadowDiagonalGradient,
		OutlineDiagonalGradient,
	}
	...
	//sd
	[HideInInspector][SerializeField] Color mEffectColorBottom = Color.black;
    ...
    //sd
	[HideInInspector][SerializeField] bool mDiagonalGradient = false;
	...
	//sd
	[HideInInspector][SerializeField] float mSpacingX = 0;
    ...
    //sd diagonal
	/// <summary>
	/// Whether a diagonal gradient will be applied.
	/// </summary>

	public bool diagonalGradient
	{
		get
		{
            return mDiagonalGradient;
		}
		set
		{
            if (mDiagonalGradient != value)
			{
                mDiagonalGradient = value;
				MarkAsChanged();
			}
		}
	}
	...
	public float spacingX
	{
		...
		set
		{
			if (Math.Abs(mSpacingX - value) > float.Epsilon)
			{
				mSpacingX = value;
				MarkAsChanged();
			}
	...
	//sd
	public Color effectColorBottom
	{
		get
		{
            return mEffectColorBottom;
		}
		set
		{
            if (mEffectColorBottom != value)
			{
                mEffectColorBottom = value;
				if (mEffectStyle != Effect.None) shouldBeProcessed = true;
			}
		}
	}
...
//sd:
		...
		{
			if (gameObject.GetComponent<LabelFontStyle>() == null)
			{
				gameObject.AddComponent<LabelFontStyle>();
			}
		}
#endif
	...
	public string GetWordAtCharacterIndex (int characterIndex)
	{
		...
		{
			//sd
			int wordStart = Utils.LastIndexOfAny(mText, new char[] {' ', '\n'}, characterIndex) + 1;
			int wordEnd = Utils.IndexOfAny(mText, new char[] { ' ', '\n', ',', '.' }, characterIndex);
	...
	public override void OnFill (BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		if (effectStyle != Effect.None)
		{
			...
			//sd = outlien with gradient
			if (effectStyle == Effect.Outline || effectStyle == Effect.OutlineGradient || effectStyle == Effect.OutlineDiagonalGradient)
			{
				offset = end;
				end = verts.size;
				...
				//SD - horizontal or vertical shadow, if shadow X or Y != 0
				if (Math.Abs(pos.x - 0) > 2)
				{
					offset = end;
					end = verts.size;
	...
	public void ApplyShadow (BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols, int start, int end, float x, float y)
	{
		...
		//SD: shadow gradient
		Color cb = mEffectColorBottom;
		cb.a *= finalAlpha;
		Color32 colb = (bitmapFont != null && bitmapFont.premultipliedAlphaShader) ? NGUITools.ApplyPMA(cb) : cb;
		...
		for (int i = start; i < end; ++i)
		{
			...
			//sd: for no icons
			if (uc.r == 254 && uc.g == 255 && uc.b == 254){uc.a = 0;}


			//sd gradient for shadow
			bool addGradient = false;
			if (effectStyle == Effect.OutlineGradient || effectStyle == Effect.ShadowGradient || effectStyle == Effect.OutlineDiagonalGradient || effectStyle == Effect.ShadowDiagonalGradient)
			{
				//if (i % 2 == 0) - po diagonali
				//if (i % 4 == 0 || i % 4 == 1) - vertical gradient
				switch (effectStyle)
				{
					case Effect.ShadowGradient:
					case Effect.OutlineGradient:
						if (i % 4 == 3 || i % 4 == 0)
						{
							addGradient = true;
						}
						break;
					case Effect.ShadowDiagonalGradient:
					case Effect.OutlineDiagonalGradient:
						if (i % 2 == 0)
						{
							addGradient = true;
						}
	...
	public void UpdateNGUIText ()
	{
        ...
        NGUIText.diagonalGradient = mDiagonalGradient && (mFont == null || !mFont.packedFontShader);
