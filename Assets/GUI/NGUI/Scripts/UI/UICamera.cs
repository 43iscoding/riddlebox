...
public class UICamera : MonoBehaviour
{
	...
	public class MouseOrTouch
	{
		...
		public bool isOverUI
		{
			get
			{
				return current != null && UICamera.FindInParents<UIRoot>(current) != null;
			}
		}
	}
...
#if UNITY_4_3
			return FindInParents<UIRoot>(hoveredObject);
#else
		...
		for (int i = 0; i < list.size; ++i)
		{
			...
			if (cam.eventType == EventType.World_3D)
			{
				if (Physics.Raycast(ray, out lastHit, dist, mask))
				{
					...
					Rigidbody rb = UICamera.FindInParents<Rigidbody>(hoveredObject);
			...
			else if (cam.eventType == EventType.World_2D)
			{
				if (m2DPlane.Raycast(ray, out dist))
				{
					...
					if (c2d)
					{
						...
						Rigidbody2D rb = UICamera.FindInParents<Rigidbody2D>(hoveredObject);
						if (rb != null) hoveredObject = rb.gameObject;
						return true;
					}
				}
