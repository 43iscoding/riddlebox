...
public static class Localization
{
	...
	//sd: detect language
	static public Func<string[], string> detectLanguage;
	static public Func<string, string> updateLanguage;
	...
	static public string language
	{
		get
		{
			if (string.IsNullOrEmpty(mLanguage))
			{
				...
				//SD: language detection
				if (detectLanguage != null)
				{
					mLanguage = detectLanguage(lan);
				}
				else
				{
...
#if UNITY_EDITOR
		//SD added info
		Debug.LogWarning("Localization key not found: '" + key + "' in " + mLanguage);
#endif
		return key;
	}
