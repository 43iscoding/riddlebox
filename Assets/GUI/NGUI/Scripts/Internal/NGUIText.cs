...
static public class NGUIText
{
    ...
    //sd
    static public bool diagonalGradient = false;
	...
	static public void Update (bool request)
	{
		...
		{
			//sd: dynamic fonts
			var fontStyle2 = fontStyle;
			var finalSize2 = finalSize;
			...
			dynamicFont.RequestCharactersInTexture(")_-", finalSize2, fontStyle2);

			if (!dynamicFont.GetCharacterInfo(')', out mTempChar, finalSize2, fontStyle2) || mTempChar.vert.height == 0f)
			{
				dynamicFont.RequestCharactersInTexture("A", finalSize2, fontStyle2);
				{
					if (!dynamicFont.GetCharacterInfo('A', out mTempChar, finalSize2, fontStyle2))
					{
						baseline = 0f;
						return;
					}
	...
	static public float GetGlyphWidth (int ch, int prev)
	{
		...
		else if (dynamicFont != null)
		{
			...
			//sd: work with custom set
			bool characterInfo = false;
			if (dynamicFont.dynamic)
			{
				characterInfo = dynamicFont.GetCharacterInfo((char)ch, out mTempChar, finalSize, fontStyle);
			}
			else
			{
				characterInfo = dynamicFont.GetCharacterInfo((char)ch, out mTempChar);
			}
			if (characterInfo)
		...
		for (int i = 0; i < textLength; ++i)
		{
			...
			if (symbol != null)
			{
				...
				if (cols != null)
				{
					...
					//sd:
					else if (symbolStyle == SymbolStyle.NormalNoEffects)
					{
                        Color32 col = new Color32(254, 255, 254, uc.a);
